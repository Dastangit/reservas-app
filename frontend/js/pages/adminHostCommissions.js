  ...
  async render() {
    ...
    return `
      <div class="admin-host-commissions-page">
        <div class="container">
          <h1>${t('admin.hostCommissionsTitle')}</h1>
          <p class="page-subtitle">${t('admin.hostCommissionsSubtitle')}</p>
      ...
      list.innerHTML = commissions.map(c => `
        <div class="commission-item commission-status-${c.status}">
          <div class="commission-header">
            <span class="commission-host">${c.host_id?.name || i18n.t('admin.hostUnknownFallback')}</span>
          ...
          <div class="commission-body">
            <p><strong>${i18n.t('admin.totalBookingsCashLabel')}</strong> $${c.total_amount} USD</p>
            <p><strong>${i18n.t('admin.commissionLabel')}</strong> $${c.commission_amount} USD</p>
            <p><strong>${i18n.t('admin.bookingsIncludedLabel')}</strong> ${c.bookings?.length || 0}</p>
            ${c.notes ? `<p><strong>${i18n.t('admin.notesLabel')}</strong> ${c.notes}</p>` : ''}
          </div>
          ${!['paid', 'waived'].includes(c.status) ? `
            <div class="commission-actions">
              <button onclick="notifyHostCommissionWhatsApp('${c._id}')" class="btn btn-outline btn-sm">${i18n.t('admin.whatsappHostBtn')}</button>
              <button onclick="markCommissionPaid('${c._id}')" class="btn btn-success btn-sm">${i18n.t('admin.markPaidBtn')}</button>
              <button onclick="waiveCommission('${c._id}')" class="btn btn-outline btn-sm">${i18n.t('admin.waiveBtn')}</button>
            </div>
          ` : ''}
        </div>
      `).join('');
